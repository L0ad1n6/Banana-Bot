...
def main():
    df = pd.read_csv("src/data/generator/raw_dataset.csv")
    sentences = df.iloc[:, 0].values
    sentences = map(b10, sentences)

    states = df.iloc[:, 1].values

    pd.DataFrame(list(zip(sentences, states)), columns=("sentence", "state")).to_csv("src/data/dataset/dataset.csv", index=False)
